...
FACES_LIBRARY_PATH = os.path.join(os.path.dirname(__file__), "faces_library.json")
FACES_OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "faces_output.json")
...
def load_faces_context() -> str:
    """Load faces library and output as context for the model."""
    context = "\n\n---\nFACES REFERENCE DATA\n\n"
    
    with open(FACES_LIBRARY_PATH, "r") as f:
        library = json.load(f)
    context += "FACES LIBRARY (for matching — DMES prototype coordinates):\n"
    for face in library["faces"]:
        context += f"\n{face['label']} ({face['face_id']}):\n"
        context += f"  D: {face['prototype_dmes']['D']}\n"
        context += f"  M: {face['prototype_dmes']['M']}\n"
        context += f"  E: {face['prototype_dmes']['E']}\n"
        context += f"  S: {face['prototype_dmes']['S']}\n"
        context += f"  Typical antipode: {face['typical_antipode']}\n"

    with open(FACES_OUTPUT_PATH, "r") as f:
        output = json.load(f)
    context += "\n\nFACES OUTPUT (for map presentation — labels, descriptions, URLs):\n"
    for face in output["faces"]:
        context += f"\n{face['label']}: {face['short_description']} → {face['url']}\n"
...
def run():
    ...
    system_prompt = load_system_prompt() + load_faces_context()
